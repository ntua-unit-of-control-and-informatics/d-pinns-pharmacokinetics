...
def custom_compile_tensorflow(self, lr, loss_fn, decay, loss_weights=None):
    """tensorflow"""
    ...
    def outputs_losses(training, inputs, targets, auxiliary_vars, losses_fn):
        ...
        aux = (
            [self.net]
            if hasattr(config, "autodiff") and config.autodiff == "forward"
            else None
        )
    ...
    def train_step(inputs, targets, auxiliary_vars):
        ...
        clipped_grads = [tf.clip_by_norm(grad, 1) for grad in grads_no_nan]
...
class FNN_custom_PositiveOutput(NN):
    ...
    def call(self, inputs, training=False):
        ...
        # y = tf.nn.relu(self.output_layer(y)) + 1e-5
...
class FNN_custom_SigmoidOutput(NN):
    """Fully-connected neural network with strictly positive outputs via softplus."""
    ...
    def call(self, inputs, training=False):
        ...
        # Final output with softplus activation to enforce positivity
        y = tf.math.sigmoid(self.output_layer(y) / 100000.0) + 1e-10
